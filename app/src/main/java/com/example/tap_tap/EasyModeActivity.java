...
public class EasyModeActivity extends Activity {
    ...
    protected void onStart() {
        ...
        while(i<model_easy_game.getTrack_part_array().length) {
            if(!model_easy_game.getState_paused()){
                int lay = model_easy_game.getTrack_part_array()[i][0];
                int note_time = model_easy_game.getTrack_part_array()[i][1];
                int note_height = model_easy_game.getTrack_part_array()[i][2];
                switch (lay) {
                    case 1: {
            ...
            new AlertDialog.Builder(this)
                    ...
                    .setPositiveButton(android.R.string.yes, new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int which) {
                            startActivity(new Intent(EasyModeActivity.this, PickActivity.class));
    ...
    public void startAnimation(TextView rectangle, ConstraintLayout layout ){
        ...
        if(model_easy_game.getState_paused()) translationY.pause();
    ...
    private void startTimer(long timerStartFrom) {
        progress_bar_timer =new CountDownTimer(63*1000, 1000) {
            ...
            public void onTick(long millisUntilFinished) {
                prog++;
                int level=prog*1000*100/model_easy_game.getTrack_duration();
